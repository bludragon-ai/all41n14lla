...
### Added
- **`consolidate` is real (was a v0.1 stub).** New `all41n14lla.engine.pathways.consolidate`, wired into the MCP `consolidate` tool and the `all41n14lla consolidate` CLI (`--threshold`, `--promote/--no-promote`, `--dry-run`). It (1) rebuilds the `edges` co-occurrence graph from shared tags across all nodes — every pair sharing N semantic tags gets weight N — so `inspect` finally returns real neighbor data; (2) decays edges that no longer co-occur by 0.9 for every 30 days since last seen, advancing `updated` so re-runs don't double-decay, and prunes only orphan edges whose node was deleted (decayed edges are reduced, never deleted); (3) promotes `concept` pairs sharing `>= threshold` tags (default 3.0) into new draft `pattern` nodes tagged `auto-generated, consolidate, unreviewed` with the two source concept ids in `links`, skipping any pair an existing pattern already links. Idempotent (SET, not increment); never modifies or deletes an existing node; auto-generated patterns are excluded from co-occurrence so the graph converges. Returns a real summary (`nodes_scanned, by_type, edges_written, edges_decayed, edges_pruned, candidates, promotions`). Stores edges in the existing SQLite `edges` table (not a separate `pathways.json`), since `inspect` already reads that table.
